...
case class Player(name: String, body: Set[Point], griDims: Dimensions , isReady: Boolean,
  ...
  private def _move(offset: Point): Player = {
    ...
    if(newBody.size == body.size) Player(name, newBody, griDims, isReady, isMovingUp, isMovingDown, score)
    else this
  }
  ...
  def setMovingUp(mvu: Boolean): Player =
    Player(name, body, griDims, isReady, isMovingUp = mvu, isMovingDown, score)

  def setMovingDown(mvd: Boolean): Player =
    Player(name, body, griDims, isReady, isMovingUp, isMovingDown = mvd, score)

  def setAsReady(): Player = Player(name, body, griDims, isReady = true, isMovingUp, isMovingDown, score)

  def incrementScore(): Player = Player(name, body, griDims, isReady, isMovingUp, isMovingDown, score + 1)
}

object Player{
  def apply(name: String, length: Int, spawnX: Int, griDims: Dimensions): Player = {
    ...
    Player(name, body, griDims, isReady = false, isMovingUp = false, isMovingDown = false, 0)
  }
